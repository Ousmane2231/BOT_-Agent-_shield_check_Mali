...
async function startBot(){
    ...
    const sock = makeWASocket({
        ...
        printQRInTerminal: false // IMPORTANT POUR RAILWAY
    })

    // CODE DE CONNEXION AU LIEU DU QR
    if(!sock.authState.creds.registered){
        await new Promise(resolve => setTimeout(resolve, 3000)) // attend 3s
        const code = await sock.requestPairingCode(OWNER)
        console.log('====================================')
        console.log('TON CODE DE CONNEXION : ' + code)
        console.log('Va sur WhatsApp > Appareils connectés > Associer avec le numéro')
        console.log('====================================')
    }
    ...
    sock.ev.on('connection.update', (u) => {
        ...
        if(connection === 'close'){
            ...
            console.log('Déconnecté. Reconnexion...', lastDisconnect.error)
